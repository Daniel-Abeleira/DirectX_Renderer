...
namespace dae {
		...
		m_pWindow(pWindow)
	{
		...
		m_pVehicleMesh->UpdateMatrices(m_pCamera.get());
		...
		m_pFireMesh->UpdateMatrices(m_pCamera.get());
	...
	void Renderer::Render() const
	{
		...
		ColorRGB bgColor{ .39f, .59f, .93f }; //Cornflower blue
		m_pDeviceContext->ClearRenderTargetView(m_pRenderTargetView, &bgColor.r);
	...
	void Renderer::ToggleRotation()
	{
		...
		std::cout << "Rotation: " << (m_RotationEnabled ? "enabled\n" : "disabled\n");
	}
	...
	void Renderer::ToggleUseNormalMap()
	{
		...
		std::cout << "Normal map: " << (m_UseNormalMap ? "enabled\n" : "disabled\n");
	}

	void Renderer::ToggleFireMesh()
	{
		m_FireEnabled = !m_FireEnabled;
		std::cout << "Fire mesh: " << (m_FireEnabled ? "enabled\n" : "disabled\n");
	}
