...
EffectBase::EffectBase(ID3D11Device* pDevice, const std::wstring& assetFile)
{
	...
	{
		std::cout <<"WorldViewProjection matrix not valid\n";
	}
	...
	{
		std::cout <<"Sampler state not valid\n";
	}
...
void EffectBase::UpdateMatrices(dae::Camera* pCamera, const dae::Matrix& worldMatrix)
{
...
void EffectBase::SetSamplerState(ID3D11SamplerState* pSamplerState)
{
	HRESULT hr{ m_pSamplerStateVariable->SetSampler(0, pSamplerState) };
	if (FAILED(hr)) std::cout <<"Failed to change sample state";
}
